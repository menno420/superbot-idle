...
suite 1594 passed, 1 skipped sb-free (see stability baseline).*
...
- **theme-gate** (`tools/theme_gate.py`, required CI check): JSON-Schema
  ...
  duplicate YAML mapping keys rejected (a strict loader fails loud instead of
...
- **Test suite: 1594 passing, 1 skipped sb-free; the pinned-host CI job runs
  ...
  tests on every PR (`1609 passed` = the 1594 sb-free plus the 15 manifest

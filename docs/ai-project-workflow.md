# superbot-idle — AI project workflow
...
5. **Verify** — run `python3 -m pytest -q && python3 bootstrap.py check --strict (theme packs additionally validate via the theme-gate step once ORDER 000 lands it in CI)` before shipping.

# superbot-idle — collaboration model
...
- When a doc and a source file disagree: Source and schema win over prose: engine code + the theme-manifest schema + theme-gate CI are ground truth; when a doc disagrees with them, fix the doc on sight and note it in the session card. Fleet instruction text is canonical at fleet-manager projects/ (version-stamped); a stale local copy loses.
- Staleness review cadence: Heartbeat control/status.md as every session's deliberate last step; every doc carries a status badge; any doc claiming live state gets re-verified against HEAD before reuse; the fleet manager's staleness sweep is the external check.

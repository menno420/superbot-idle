...
Roll a substrate-kit release out to superbot-idle — one target repo of the
...
7. Verify + flip — `python3 -m pytest -q && python3 bootstrap.py check --strict (theme packs additionally validate via the theme-gate step once ORDER 000 lands it in CI)` and
